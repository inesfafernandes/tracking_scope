...
namespace XPSApplicationTest
{
    public partial class FormApplicationXPS : Form
    {
        ...
        string   m_PositionerName2;
        string gpioname;
        string gpioname2;
        bool     m_IsPositioner;
        double[] m_TargetPosition = new double[NB_POSITIONERS];
        double[] m_TargetPosition2 = new double[NB_POSITIONERS];
        double[] m_CurrentPosition = new double[NB_POSITIONERS];
        double[] m_CurrentPosition2 = new double[NB_POSITIONERS];
        ...
        public FormApplicationXPS()
        {
            ...
            gpioname2 = string.Format("GPIO4.ADC2");
            m_pollingFlag = false;
        ...
        public void UpdateGroupStatus()
        {
            try
            {
                ...
                if (m_xpsInterfaceForPolling != null)
                {
                    ...
                    result = m_xpsInterfaceForPolling.GroupStatusGet(m_GroupName2, out m_CurrentGroupStatus, out errorString);
                    if (result == CommandInterfaceXPS.XPS.FAILURE) // Communication failure with XPS 
                    {
                        m_CurrentGroupStatus = 0;
                        if (errorString.Length > 0)
        ...
        private void ConnectButton(object sender, EventArgs e)
        {
            ...
            m_PositionerName2 = TextBox_Group2.Text;
            int index = m_PositionerName.LastIndexOf('.');
            if (index != -1)
            {
                m_IsPositioner = true;
                m_GroupName = m_PositionerName.Substring(0, index);
                m_GroupName2 = m_PositionerName2.Substring(0, index);
            ...
            else
            {
                ...
                m_GroupName2 = m_PositionerName2;
                ...
                if (m_xpsInterface != null)
                {
                    ...
                    if (result == CommandInterfaceXPS.XPS.FAILURE) // Communication failure with XPS 
                    {
                        if (errorString.Length > 0)
                        {
                            ...
                            int.TryParse(errorString, out errorCode);
                            m_xpsInterface.ErrorStringGet(errorCode, out m_errorDescription, out errorString);
                           //MessageBox.Show(m_errorDescription);
                            label_ErrorMessage.Text = string.Format("GroupInitialize ERROR {0}: {1} {2}", result, m_errorDescription, errorString);
                        }
                        else
                            label_ErrorMessage.Text = string.Format("Communication failure with XPS after GroupInitialize ");
                    }
            ...
            try
            {
                ...
                if (m_IsPositioner == true)
                {
                    ...
                    if ((m_xpsInterface != null) && (m_CommunicationOK == true))
                    {
                        ...
                        result = m_xpsInterface.EventExtendedStart(out pos1_AOEventID, out errorString);
                ...
                if (m_IsPositioner == true)
                {
                    ...
                    if ((m_xpsInterface != null) && (m_CommunicationOK == true))
                    {
                        ...
                        //{
                            ...
                            result = m_xpsInterface.EventExtendedRemove(pos1_AOEventID, out errorString);
                        //}
        ...
        private void EnableVTrkButton_Click(object sender, EventArgs e)
        {
            try
            {
                label_ErrorMessage.Text = string.Empty;
                ...
                if (m_IsPositioner == true)
                {
                    ...
                    if ((m_xpsInterface != null) && (m_CommunicationOK == true))
                    {
                        string errorString = string.Empty;
                        string statusString = string.Empty;
                        label_ErrorMessage.Text = "buttonEnableVTrk: ";
                        double trk_offset = 0.0;
                        double trk_scale = 50.0; //for position this is mm/volt for velocity I guess it will be mm/s per volt, so max velocity is 500 scale should be 50?
                        double trk_vel = 500.0;
                        double trk_acc = 2000.0;
                        double trk_deadband = 0.005;  // in volts
                        int trkOrder = 1; // non linearity of velocity encoding
                        // MessageBox.Show(m_PositionerName);
                        //MessageBox.Show(gpioname);

                        //int result = m_xpsInterface.PositionerAnalogTrackingPositionParametersSet(m_PositionerName, gpioname, trk_offset, trk_scale, trk_vel, trk_acc, out errorString);
                        int result = m_xpsInterface.PositionerAnalogTrackingVelocityParametersSet(m_PositionerName, gpioname, trk_offset, trk_scale,trk_deadband, trkOrder, trk_vel, trk_acc, out errorString);
                        ...
                        //MessageBox.Show(m_PositionerName);
                        String trackType = String.Format("Velocity");
                        ...
                        if (result == CommandInterfaceXPS.XPS.FAILURE) // Communication failure with XPS 
                        {
                            if (errorString.Length > 0)
                            {
                                ...
                                int.TryParse(errorString, out errorCode);
                                m_xpsInterface.ErrorStringGet(errorCode, out m_errorDescription, out errorString);
                                label_ErrorMessage.Text = string.Format("GroupVTrkEnable ERROR {0}: {1}", result, m_errorDescription);
                            }
                            else
                                label_ErrorMessage.Text = string.Format("Communication failure with XPS after EnableVTrk ");
                        }
                    }
                }
            }
        ...
        private void DisableVTrkButton_Click(object sender, EventArgs e)
        {
            try
            {
                label_ErrorMessage.Text = string.Empty;
                    ...
                    if ((m_xpsInterface != null) && (m_CommunicationOK == true))
                    {
                        ...
                        if (result == CommandInterfaceXPS.XPS.FAILURE) // Communication failure with XPS 
                        {
                            if (errorString.Length > 0)
                            {
                                ...
                                int.TryParse(errorString, out errorCode);
                                m_xpsInterface.ErrorStringGet(errorCode, out m_errorDescription, out errorString);
                                label_ErrorMessage.Text = string.Format("Vel track disable ERROR {0}: {1}", result, m_errorDescription);
                            }
                            else
                                label_ErrorMessage.Text = string.Format("Communication failure with XPS after DisableVtrk ");
                        }
                    }
                }
            }
            ...
            {
                label_ErrorMessage.Text = "Exception in buttonDisableVTrk: " + ex.Message;
            }
        }

        private void StartAOVButton_Click(object sender, EventArgs e)
        {
            try
            {
                //label_ErrorMessage.Text = string.Empty;
                ...
                if (m_IsPositioner == true)
                {
                    ...
                    {
                        string errorString = string.Empty;
                        //label_ErrorMessage.Text = "buttonStartAOVel: ";
                       
                        int result = m_xpsInterface.EventExtendedConfigurationTriggerSet(new string[] { "Always" }, new string[] { "1" }, new string[] { "0" }, new string[] { "0" }, new string[] { "0" }, out errorString);

                        result = m_xpsInterface.EventExtendedConfigurationActionSet(new string[] { "GPIO4.DAC1.DACSet.CurrentVelocity" }, new string[] { m_PositionerName }, new string[] { "200" }, new string[] { "0" }, new string[] { "0" }, out errorString);


                        result = m_xpsInterface.EventExtendedStart(out vel1_AOEventID, out errorString);
                        ...
                        if (result == CommandInterfaceXPS.XPS.FAILURE) // Communication failure with XPS 
                        {
                            if (errorString.Length > 0)
                            {
                                ...
                                int.TryParse(errorString, out errorCode);
                                m_xpsInterface.ErrorStringGet(errorCode, out m_errorDescription, out errorString);
                                //label_ErrorMessage.Text = string.Format("GroupSetAOV ERROR {0}: {1}", result, m_errorDescription);
                            }
                            //else
                                //label_ErrorMessage.Text = string.Format("Communication failure with XPS after EnableAoVel ");
                        }
                    }
                }
            }
            ...
            {
                //label_ErrorMessage.Text = "Exception in buttonEnableAoVel: " + ex.Message;
            }
        }

        private void StopAOVButton_Click(object sender, EventArgs e)
        {
            try
            {
                label_ErrorMessage.Text = string.Empty;
                ...
                if (m_IsPositioner == true)
                {
                    ...
                    {
                        string errorString = string.Empty;
                        label_ErrorMessage.Text = "buttonDisableAOVel: ";
                        //m_xpsInterface
                        int result = 0;


                            result = m_xpsInterface.EventExtendedRemove(vel1_AOEventID, out errorString);
                        ...
                        if (result == CommandInterfaceXPS.XPS.FAILURE) // Communication failure with XPS 
                        {
                            if (errorString.Length > 0)
                            {
                                ...
                                int.TryParse(errorString, out errorCode);
                                m_xpsInterface.ErrorStringGet(errorCode, out m_errorDescription, out errorString);
                                label_ErrorMessage.Text = string.Format("GroupDisableAOVel ERROR {0}: {1}", result, m_errorDescription);
                            }
                            else
                                label_ErrorMessage.Text = string.Format("Communication failure with XPS after GroupDisableAOVel ");
                        }
                    }
                }
            }
            ...
            {
                label_ErrorMessage.Text = "Exception in GroupDisableAOVel: " + ex.Message;
            }
        }
        ...
        private void ButtonInitialize2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string errorString = string.Empty;
                    int result = m_xpsInterface.GroupInitialize(m_GroupName2, out errorString);
                    if (result == CommandInterfaceXPS.XPS.FAILURE) // Communication failure with XPS 
                    {
                        if (errorString.Length > 0)
                        {
            ...
            {
                label_ErrorMessage.Text = "Exception in buttonInitialize2_Click: " + ex.Message;
            }
        }
        ...
        private void ButtonHome2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string errorString = string.Empty;
                    int result = m_xpsInterface.GroupHomeSearch(m_GroupName2, out errorString);
                    if (result == CommandInterfaceXPS.XPS.FAILURE) // Communication failure with XPS 
                    {
                        if (errorString.Length > 0)
                        {
        ...
        private void ButtonMoveTo2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (m_IsPositioner == true)
                {
                    double.TryParse(textBoxTarget2.Text, out m_TargetPosition2[0]);
                    if ((m_xpsInterface != null) && (m_CommunicationOK == true))
                    {
                        string errorString = string.Empty;
                        int result = m_xpsInterface.GroupMoveAbsolute(m_PositionerName2, m_TargetPosition2, 1, out errorString);
                        if (result == CommandInterfaceXPS.XPS.FAILURE) // Communication failure with XPS 
                        {
                            if (errorString.Length > 0)
                            {
        ...
        private void ButtonKill2_Click(object sender, EventArgs e)
        {
            try
            {
                label_ErrorMessage.Text = string.Empty;
                ...
                {
                    string errorString = string.Empty;
                    int result = m_xpsInterface.GroupKill(m_GroupName2, out errorString);
                    if (result == CommandInterfaceXPS.XPS.FAILURE) // Communication failure with XPS 
                    {
                        if (errorString.Length > 0)
                        {
        ...
        private void EnablePTrkButton2_Click(object sender, EventArgs e)
        {
            try
            {
                label_ErrorMessage.Text = string.Empty;
                ...
                if (m_IsPositioner == true)
                {
                    ...
                    if ((m_xpsInterface != null) && (m_CommunicationOK == true))
                    {
                        ...
                        string statusString = string.Empty;
                        int hardwareStatus = 0;
                        label_ErrorMessage.Text = "buttonEnablePTrk2: ";
                        double trk_offset = 0.0;
                        double trk_scale = 10.0;
                        double trk_vel = 500.0;
                        double trk_acc = 2000.0;
                        // MessageBox.Show(m_PositionerName);
                        //MessageBox.Show(gpioname);
                        int result = m_xpsInterface.PositionerAnalogTrackingPositionParametersSet(m_PositionerName2, gpioname2, trk_offset, trk_scale, trk_vel, trk_acc, out errorString);
                        ...
                        result = m_xpsInterface.GroupAnalogTrackingModeEnable("Group2", trackType, out errorString);
                        //int result = m_xpsInterface.GroupMoveAbsolute(m_PositionerName, m_TargetPosition, 1, out errorString);
                        if (result == CommandInterfaceXPS.XPS.FAILURE) // Communication failure with XPS 
                        {
                            if (errorString.Length > 0)
            ...
            {
                label_ErrorMessage.Text = "Exception in buttonEnablePTrk2: " + ex.Message;
            }
        }

        private void DisablePTrkButton2_Click(object sender, EventArgs e)
        {
            try
            {
                label_ErrorMessage.Text = string.Empty;
                ...
                if (m_IsPositioner == true)
                {
                    ...
                    if ((m_xpsInterface != null) && (m_CommunicationOK == true))
                    {
                        ...
                        int result = m_xpsInterface.GroupAnalogTrackingModeDisable(m_GroupName2, out errorString);
                        //int result = m_xpsInterface.GroupMoveAbsolute(m_PositionerName, m_TargetPosition, 1, out errorString);
                        if (result == CommandInterfaceXPS.XPS.FAILURE) // Communication failure with XPS 
                        {
                            if (errorString.Length > 0)
                            {
                                ...
                                label_ErrorMessage.Text = string.Format("Communication failure with XPS after DisablePTrk2 ");
                        }
                    }
                }
            }
            ...
            {
                label_ErrorMessage.Text = "Exception in buttonDisablePTrk2: " + ex.Message;
            }
        }

        private void StartAOPButton2_Click(object sender, EventArgs e)
        {
            try
            {
                label_ErrorMessage.Text = string.Empty;
                ...
                if (m_IsPositioner == true)
                {
                    ...
                    {
                        string errorString = string.Empty;
                        label_ErrorMessage.Text = "buttonStartAOPos2: ";

                        int result = m_xpsInterface.EventExtendedConfigurationTriggerSet(new string[] { "Always" }, new string[] { "1" }, new string[] { "0" }, new string[] { "0" }, new string[] { "0" }, out errorString);

                        result = m_xpsInterface.EventExtendedConfigurationActionSet(new string[] { "GPIO4.DAC2.DACSet.CurrentPosition" }, new string[] { m_PositionerName2 }, new string[] { "0.1" }, new string[] { "0" }, new string[] { "0" }, out errorString);

                        result = m_xpsInterface.EventExtendedStart(out pos2_AOEventID, out errorString);
        ...
        private void StopAOPButton2_Click(object sender, EventArgs e)
        {
            try
            {
                label_ErrorMessage.Text = string.Empty;
                ...
                if (m_IsPositioner == true)
                {
                    ...
                    {
                        string errorString = string.Empty;
                        label_ErrorMessage.Text = "buttonStopAOPos2: ";
                        //m_xpsInterface
                        int result = 0;
                        ...
                        result = m_xpsInterface.EventExtendedRemove(pos2_AOEventID, out errorString);
                        //}
                        ...
                        if (result == CommandInterfaceXPS.XPS.FAILURE) // Communication failure with XPS 
                        {
                            if (errorString.Length > 0)
                            {
                                ...
                                label_ErrorMessage.Text = string.Format("Communication failure with XPS after StopAOPos2 ");
                        }
                    }
                }
            }
            ...
            {
                label_ErrorMessage.Text = "Exception in StopAOPos2: " + ex.Message;
            }
        }
